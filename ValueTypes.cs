...
namespace ValueTypes
{
    class TypeExercise
    {
        ...
        /// </summary>        
        public string? BerechneFlaeche(double? laenge, double? breite)
        {
                      
            if(laenge != null && breite != null)
            {
               return (laenge * breite).ToString();
            } else
            {
                return "";
            }
        ...
        {
            return (double)((laenge ?? 0) * (breite ?? 0));
        }
        ...
        {
            return nettopreis * (steuerProdukt ?? steuerKategorie ?? (decimal)1.2);
        }
